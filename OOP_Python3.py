...
# ===============================================================
# Updating the OOP_song - Removing circular references
# ===============================================================

# We will remove the circular references from our objects
# See "Flowchart_4 (Remove circular references)"

# We will modify class Song as shown below

# Changes to make:
# MODIFY the program so that the class structure matches the simplified program (Flowchart_4).
# Artist object can hold references to Album objects, and Album objects can hold references to Song objects.
# But there must be no circular references.

# Change_1 => Modify the Song docstring to reflect the Song object will now store the name of the artist rather
#             than a reference to the artist object.

# Change_2 => Remove the circular references between Album and Artist.

# Change_3 => Change Artist class so when it creates new album, it is using its name rather than passing "self"
#             Hence artist name will be storing artist as a string rather than storing a reference to the Artist object.
...
class Song:
    ...
    Attributes:
        ...
        artist (str): The name of the songs creator (Change_1)
    ...
    def __init__(self, title, artist, duration=0):
        ...
        self.title = title  # NOTE change this back to "self.title = title" instead of "self.name = title"
        self.artist = artist
    ...
    # Now we use a getter function to assign self.title above to self.name
    def get_title(self):
        return self.title  # it returns self.title
    # Here we use a read only property called "name" that uses the getter (get_title) to return the value of the title attribute
    # This way, existing code e.g. create_checkfile function will still work but we will still be able to use the find_object as well
...
class Album:
    ...
    Attributes:
        ...
        artist (str): The name of the artist responsible for the album, if not specified, (Change_2)
    ...
    def __init__(self, name, year, artist=None):
        ...
        self.year = year
        if artist is None:
            self.artist = "Various Artists"  # Change_2 (we add string "Various Artists" instead of referencing to Artist object
        else:
            self.artist = artist
